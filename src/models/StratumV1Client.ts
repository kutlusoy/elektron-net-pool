...
// BIP320 version-rolling bits 13-16 (0x0001e000), reserved for the pool's
// own work rotation (see sendNewMiningJob). The miner's advertised mask
// (ConfigurationMessage) is the complementary 0x1ffe0000, so a compliant
// miner's own version-rolling can never touch these bits. Rotating them on
// every job (real GBT refresh or local work refresh) gives each job a
// distinct header search space without needing a fresh coinbase/extranonce,
// which is what lets local work refreshes run every WORK_REFRESH_INTERVAL_MS
// without hitting getblocktemplate at all.
...
export class StratumV1Client {
    ...
    // BIP320 pool-side work rotation state (see sendNewMiningJob /
    // refreshLocalMiningWork). currentJobTemplate is the raw, un-rotated
    // template from the last real getblocktemplate call; every call to
    // sendNewMiningJob computes its own rotated version on top of it, so
    // this field is never itself mutated with rotation bits baked in.
    ...
    constructor(
        ...
        // Stop all work associated with this miner as soon as the TCP
        // connection disappears, however it disappears. destroy() is
        // idempotent, so both handlers firing (or firing alongside an
        // explicit closeSocket()/write() failure) is harmless.
    ...
    private async refreshMiningJob() {
        ...
        // Cache the raw (un-rotated) template for the cheap local work
        // refreshes below; sendNewMiningJob computes its own rotated version
        // on top of it each time, so this stays the template's true base.
    ...
    // Issues a fresh Stratum job from the already-cached template without
    // calling getblocktemplate: only nTime advances (to real time) and
    // sendNewMiningJob's BIP320 rotation gives the job a distinct header
    // search space. This is what lets ASICs get new work every
    // WORK_REFRESH_INTERVAL_MS without the pool's RPC load scaling with it.
    private async refreshLocalMiningWork() {
        ...
        // A real GBT refresh (new block or the 30s timer) may have just run
        // and already sent a job; avoid an almost-immediate duplicate.
        ...
        // from the BIP320 rotation instead.
    ...
    private async sendNewMiningJob(jobTemplate: IJobTemplate) {
        ...
        // Give this job a distinct header search space by rotating the
        // pool-reserved BIP320 bits (see ELEKTRON_POOL_VERSION_BITS_MASK).
        // Reset the rotation on a real new-block boundary (clearJobs) so a
        // fresh block starts from a known state; every other call (real GBT
        // refresh or local work refresh) just advances it.
        if (jobTemplate.blockData.clearJobs) {
    ...
    private async checkDifficulty() {
        ...
        if (targetDiff != this.sessionDifficulty) {
            ...
            // Reuse the cached template if a real refresh already fetched one
            // recently rather than issuing a redundant getblocktemplate call;
            // refreshLocalMiningWork/refreshMiningJob keep this current.
            const jobTemplate = this.currentJobTemplate
                ?? await this.stratumV1JobsService.buildTemplateFor(this.clientAuthorization.address);
            this.currentJobTemplate = jobTemplate;
    ...
    private async write(message: string): Promise<boolean> {
        // A background work-refresh tick may already be in flight when the
        // miner disconnects; that is an expected race, not an error.
        if (this.isSocketClosed()) {
            return false;
        }
        ...
        try {
            ...
            // Normal miner disconnect/reconnect races stay silent; genuinely
            // unexpected socket errors are still logged.
