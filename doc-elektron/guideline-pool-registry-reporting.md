# Elektron Net - `elektron-net-pool` Pool Registry Reporting Guideline
...
- **Reference implementation:** `src/app.controller.ts` (the existing `GET /pool/identity` endpoint this extends)
- **See also:** [`fix-report-pool-identity-utxo-attestation.md`](./fix-report-pool-identity-utxo-attestation.md) (why the on-chain approach this replaces was reverted), the identical planning document in `elektron-net-ppool` (this document mirrors it byte for byte apart from repo-specific paths), `elektron-net-mempool`'s `doc-elektron/guideline-pool-registry-reporting.md` (the consumer side)
...
This binds trust to control of the registered URL, not to a wallet or on-chain data, so it works the same way for this repo and `elektron-net-ppool` alike.
...
- **`GET /pool/identity/confirm?blockHash=<hex>`** (new, `app.controller.ts`, alongside the existing `GET /pool/identity`): returns `{ confirmed, name, url }`, where `confirmed` is true only if this pool itself recorded finding that exact block hash recently.
...
1. Report endpoint: `POST /api/v1/pool-registry/report`, body `{ name, blockHash }` (mempool side). Confirm endpoint: `GET /pool/identity/confirm?blockHash=<hex>` (this side), response `{ confirmed, name, url }`. Identical in `elektron-net-ppool`.
...
- [x] Verified byte-identical behavior with `elektron-net-ppool`'s implementation
